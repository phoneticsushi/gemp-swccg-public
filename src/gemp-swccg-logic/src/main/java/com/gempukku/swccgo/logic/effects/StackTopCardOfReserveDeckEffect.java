...
public class StackTopCardOfReserveDeckEffect extends AbstractStandardEffect {
    ...
    protected FullEffectResult playEffectReturningResult(SwccgGame game) {
        if (isPlayableInFull(game)) {
            ...
            if (card != null) {
                if (_faceDown)
                    game.getGameState().sendMessage(_playerId + " stacks top card of Reserve Deck on " + GameUtils.getCardLink(_stackOn));
                else
                    game.getGameState().sendMessage(_playerId + " stacks " + GameUtils.getCardLink(card) + " from top of Reserve Deck on " + GameUtils.getCardLink(_stackOn));
