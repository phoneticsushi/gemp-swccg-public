 ...
 */
public class HitCardAndModifyPowerAndForfeitEffect extends AbstractSubActionEffect implements PreventableCardEffect {
    private PhysicalCard _cardHitAndReset;
    private float _forfeitModifierAmount;
    ...
    private HitCardAndModifyPowerAndForfeitEffect _that;
     ...
     * @param forfeitModifierAmount the forfeit modifier amount
     ...
     */
    public HitCardAndModifyPowerAndForfeitEffect(Action action, PhysicalCard cardHitAndReset, float powerModifierAmount, float forfeitModifierAmount, PhysicalCard hitByCard, SwccgBuiltInCardBlueprint hitByPermanentWeapon, PhysicalCard cardFiringWeapon) {
        super(action);
        _cardHitAndReset = cardHitAndReset;
        _forfeitModifierAmount = forfeitModifierAmount;
                                    ...
                                    protected void doPlayEffect(SwccgGame game) {
                                        if (isEffectOnCardPrevented(_cardHitAndReset) && Filters.or(Filters.onTable, Filters.canBeTargetedByWeaponAsIfPresent).accepts(game, _cardHitAndReset)) {
                                            ...
                                            if (modifyPower && modifyForfeit) {
                                                message += ", its power is reduced by " + GuiUtils.formatAsString(_powerModifierAmount) + " and its forfeit is reduced by " + GuiUtils.formatAsString(_forfeitModifierAmount);
                                            } else if (modifyPower) {
                                                message += " and its power is reduced by " + GuiUtils.formatAsString(_powerModifierAmount) + ", but its forfeit is prevented from being reduced";
                                            } else if (modifyForfeit) {
                                                message += " and its forfeit is reduced by " + GuiUtils.formatAsString(_forfeitModifierAmount) + ", but its power is prevented from being reduced";
                                            }
                                            ...
                                            if (modifyForfeit) {
                                                ...
                                                Modifier modifier = new ForfeitModifier(source, cardFilter, _forfeitModifierAmount);
                                                modifier.skipSettingNotRemovedOnRestoreToNormal();
