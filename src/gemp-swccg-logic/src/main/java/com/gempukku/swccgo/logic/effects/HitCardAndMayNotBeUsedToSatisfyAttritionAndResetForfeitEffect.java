...
public class HitCardAndMayNotBeUsedToSatisfyAttritionAndResetForfeitEffect extends AbstractSubActionEffect implements PreventableCardEffect {
    ...
    private HitCardAndMayNotBeUsedToSatisfyAttritionAndResetForfeitEffect _that;

    /**
     * Creates an effect to 'hit' a card, cause it to not be able to be used to satisfy attrition, and resets its forfeit value.
     * @param action the action performing this effect
     * @param cardHitAndReset the card that is hit and whose forfeit value is reset
     * @param resetValue the reset value
     * @param hitByCard the card was hit by
     * @param hitByPermanentWeapon the permanent weapon that hit the card
     * @param cardFiringWeapon the card that fired the weapon
     */
    public HitCardAndMayNotBeUsedToSatisfyAttritionAndResetForfeitEffect(Action action, PhysicalCard cardHitAndReset, float resetValue, PhysicalCard hitByCard, SwccgBuiltInCardBlueprint hitByPermanentWeapon, PhysicalCard cardFiringWeapon) {
        super(action);
                                    ...
                                    protected void doPlayEffect(SwccgGame game) {
                                        if (isEffectOnCardPrevented(_cardHitAndReset) && Filters.or(Filters.onTable, Filters.canBeTargetedByWeaponAsIfPresent).accepts(game, _cardHitAndReset)) {
                                            ...
                                            if (_resetValue < currentForfeit && modifiersQuerying.isProhibitedFromHavingForfeitReduced(gameState, _cardHitAndReset)) {
                                                gameState.sendMessage(GameUtils.getCardLink(_cardHitAndReset) + " is 'hit' by " + GameUtils.getCardLink(_hitByCard) + ", may not be used to satisfy attrition, but its forfeit is prevented from being reduced to " + GuiUtils.formatAsString(_resetValue));
                                                resetForfeit = false;
                                            } else {
                                                gameState.sendMessage(GameUtils.getCardLink(_cardHitAndReset) + " is 'hit' by " + GameUtils.getCardLink(_hitByCard) + ", may not be used to satisfy attrition, and its forfeit is reset to " + GuiUtils.formatAsString(_resetValue));
                                            }
                                            ...
                                            Filter attritionCardFilter = Filters.and(Filters.sameCardId(_cardHitAndReset), Filters.or(Filters.onTable, Filters.canBeTargetedByWeaponAsIfPresent));

                                            Modifier attritionModifier = new MayNotBeUsedToSatisfyAttritionModifier(source, attritionCardFilter);
                                            attritionModifier.skipSettingNotRemovedOnRestoreToNormal();
                                            ...
                                            if (gameState.isDuringBattle()
                                                ...
                                                modifiersEnvironment.addUntilEndOfBattleModifier(attritionModifier);
                                            } else {
                                                modifiersEnvironment.addUntilEndOfGameModifier(attritionModifier);
                                            }
